(function () {
  ...
  class Game{
    constructor(){
      if(reserva) container.removeChild(reserva.element);
      reserva = new Reserva();
      status = new Status();
      focos = [];
      started = true;
  ...
  class Reserva {
    constructor () {
      ...
      container.style = 'margin-top: 100px; max-width: 1278px !important;'
    ...
    for(var foc=0; foc < focos.length; foc++){
      ...
      if(state == "burn"){
        ...
        if (current_time > foco.keep_alive){
          ...
          foco.element.style.backgroundImage = "url('../img/devastacao.png')";
